...
describe('music manager - shuffle', () => {
	afterEach(() => {
		sinon.restore();
	});

	it('should shuffle successfull', async () => {
		...
		await musicManager.shuffle({ ctx });
		...
		expect(mockQueue.shuffle.mock.calls).toHaveLength(1);
	...
	it('should fail to shuffle because queue is not playing', async () => {
		const mockCtx = new MockContext({});
	...
	it('should fail to shuffle because queue is not defined', async () => {
		const mockCtx = new MockContext({});
