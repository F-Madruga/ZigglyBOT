...
describe('music manager - back', () => {
	afterEach(() => {
		sinon.restore();
	});

	it('should back successfull', async () => {
		...
		expect(mockQueue.back.mock.calls).toHaveLength(1);
		...
		expect(mockCtx.interaction.followUp.mock.calls[0][0]).toStrictEqual({
			content: 'Playing the previous music',
		});
	});

	it('should fail to back because queue is not playing', async () => {
		const mockCtx = new MockContext({});
	...
	it('should fail to back because queue is not defined', async () => {
		const mockCtx = new MockContext({});
